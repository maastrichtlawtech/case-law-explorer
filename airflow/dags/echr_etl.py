...
from definitions.storage_handler import Storage, get_path_raw, CSV_ECHR_CASES, JSON_FULL_TEXT_ECHR, TXT_ECHR_EDGES, TXT_ECHR_NODES
...
def echr_etl(**kwargs):
    ...
    if os.path.exists(output_path):
        ...
        extraction_args = [
            ...
            '--count', str(eval(Variable.get('ECHR_AMOUNT_TO_EXTRACT', default_var='1000')))
        ]
        ...
        end_time = time.time()
        logging.info(f"Time taken for ECHR extraction: {time.strftime('%H:%M:%S', time.gmtime(end_time - start_time))}")
