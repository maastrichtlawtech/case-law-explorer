...
from lido.tasks.prepare_bwbidlist import (
    task_bwbidlist_xml_to_json,
    task_unzip_bwbidlist,
)
from lido.tasks.swap_postgres import (
    task_create_staging_tables,
    task_load_csv,
    task_swap_tables,
)
...
with DAG(
    ...
    with TaskGroup("prepare_lido") as prepare_lido:
        ...
        download_lido_ttl = BashOperator(
            ...
            bash_command=("curl -sSLf {url} -o {output}").format(
                url=URL_LIDO_TTL_GZ, output=FILE_LIDO_TTL_GZ
            ),
        )
        # download_lido_ttl = EmptyOperator(task_id="skip_download_lido_ttl")

        check_lido_ttl = BashOperator(
            task_id="check_lido_ttl", bash_command=f"test -s {FILE_LIDO_TTL_GZ}"
        )
        ...
        check_laws_nt = BashOperator(
            task_id="check_laws_nt", bash_command=f"test -s {FILE_LAWS_NT}"
        )

        check_cases_nt = BashOperator(
            task_id="check_cases_nt", bash_command=f"test -s {FILE_CASES_NT}"
        )
    ...
    with TaskGroup("prepare_bwb") as prepare_bwb:
        ...
        download_bwbidlist = BashOperator(
            ...
            bash_command=("curl -sSLf {url} -o {output}").format(
                url=URL_BWB_IDS_ZIP, output=FILE_BWB_IDS_ZIP
            ),
        )

        unzip_bwbidlist = PythonOperator(
            task_id="unzip_bwbidlist", python_callable=task_unzip_bwbidlist
        )

        bwbidlist_xml_to_json = PythonOperator(
            task_id="bwbidlist_xml_to_json", python_callable=task_bwbidlist_xml_to_json
        )

        reset_bwb_data >> download_bwbidlist >> unzip_bwbidlist >> bwbidlist_xml_to_json
    ...
    with TaskGroup("to_sqlite") as to_sqlite:
        ...
        check_sqlite_db = BashOperator(
            task_id="check_sqlite_db", bash_command=f"test -s {FILE_SQLITE_DB}"
        )
        ...
        bwbidlist_to_sqlite = PythonOperator(
            task_id="bwbidlist_to_sqlite", python_callable=task_bwbidlist_to_sqlite
        )
    ...
    with TaskGroup("sqlite_to_csv") as sqlite_to_csv:
        ...
        reset_csvs >> legal_case_to_csv >> law_element_to_csv >> case_law_to_csv >> law_alias_to_csv

    with TaskGroup("csv_to_postgres") as csv_to_postgres:

        create_staging_tables = PythonOperator(
            task_id="create_staging_tables", python_callable=task_create_staging_tables
        )
        ...
        swap_tables = PythonOperator(task_id="swap_tables", python_callable=task_swap_tables)
