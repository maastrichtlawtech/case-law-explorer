...
parser.add_argument('--start_date', help='DD/MM/YYYY. Save cases from after this date.', type=str, \
                    required=False)
parser.add_argument('--end_date', help='DD-MM-YYYY. Save cases from before this date.', type=str, \
                    required=False)
...
try:
    arg_start_date = dateutil.parser.parse(args.start_date, dayfirst=True).date() if args.start_date else None
    arg_end_date = dateutil.parser.parse(args.end_date, dayfirst=True).date() if args.end_date else None
except dateutil.parser._parser.ParserError:
    print("The date formats are incorrect, they should be dd/mm/yyyy")
    sys.exit(2)
df, resultcount = read_echr_metadata(start_id=arg_start_id, end_id=arg_end_id, start_date=arg_start_date, end_date=arg_end_date)
