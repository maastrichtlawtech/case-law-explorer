...
SOURCE = LI_DOCUMENT_TYPE = ECHR_DOCUMENT_TYPE = (
    "source"  # source of case (so far: always 'Rechtspraak')
)
JURISDICTION_COUNTRY = LI_JURISDICTION = (
    "jurisdiction_country"  # country of jurisdiction (so far: always 'NL')
)
...
RS_ZAAKNUMMER = LI_CASE_NUMBER = (
    "case_number"  # case number, for internal use by courts -- main identifier prior to introduction of ECLI
)
RS_SUBJECT = LI_LAW_AREA = ECHR_ARTICLES = (
    "domains"  # domain(s) (area(s) of the law) applicable to case
)
RS_HASVERSION = LI_SOURCES = ECHR_PUBLISHED_BY = (
    "alternative_publications"  # references to other publications
)
...
RS_ISSUED = LI_PUBLICATION_DATE = CELLAR_DATE_OF_DOCUMENT = (
    "date_publication"  # date of document publication
)
RS_INHOUDSINDICATIE = LI_SUMMARY = CELLAR_SUMMARY = ECHR_CONCLUSION = "summary"  # case summary
ECLI = RS_IDENTIFIER = (
    "ECLI"  # ECLI (European Case Law Identifier) -- unique identifier for court decisions in Europe
)
...
RS_REFERENCES = ECHR_APPLICABLE_ARTICLES = (
    "referenced_legislation_titles"  # title of applicable legislation
)
...
CELLAR_AFFECTING_STRING = (
    "affecting_string"  # entire strings with more details about case affecting
)
...
ECHR_PARTICIPANTS = "extractedappno"  # applicants extracted form the report indicating the existing of the full text
