...
def rechtspraak_extract(
    ...
    base_files = [os.path.join(output_dir, f) for f in os.listdir(output_dir) if f.startswith('base_') and f.endswith('.csv')]
    if base_files:
    ...
    if not metadata_df.empty:
        citations_df = get_citations(metadata_df, os.getenv('LIDO_USERNAME'), os.getenv('LIDO_PASSWORD'), 1)
