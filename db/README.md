...
- **`public`** — Airflow's own metadata (managed by Airflow itself) and the pre-existing `lido_postgres` DAG's tables (`legal_case`, `law_element`, `case_law`, `law_alias` — see `airflow/dags/lido/`). Unrelated to `cle_v2`.
