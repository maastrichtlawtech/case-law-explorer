...
def task_bwbidlist_xml_to_json():
    ...
    for regeling in regelingLijst.iterfind("NS1:RegelingInfo", namespaces=NS):
        ...
        citeer_titels = [
            ...
            for titel in regeling.find("NS1:CiteertitelLijst", namespaces=NS).iterfind(
                "NS1:Citeertitel", namespaces=NS
            )
        ]
        afkortingen = [
            afkorting.text
            for afkorting in regeling.find("NS1:AfkortingLijst", namespaces=NS).iterfind(
                "NS1:Afkorting", namespaces=NS
            )
        ]
        titels_niet_officieel = [
            titel.text
            for titel in regeling.find("NS1:NietOfficieleTitelLijst", namespaces=NS).iterfind(
                "NS1:NietOfficieleTitel", namespaces=NS
            )
        ]
