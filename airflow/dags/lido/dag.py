...
with DAG(
    ...
    with TaskGroup('prepare_bwb') as prepare_bwb:
        ...
        reset_bwb_data = BashOperator(
            ...
            # rm files but keep dirs
            bash_command=(f'rm {DIR_DATA_BWB}/*')
        )
    ...
    with TaskGroup('sqlite_to_csv') as sqlite_to_csv:
        ...
        legal_case_to_csv = BashOperator(
            ...
            bash_command=f'sqlite3 {FILE_SQLITE_DB} -header -csv "SELECT * FROM legal_case;" > {FILE_CASES_CSV}'
        )

        law_element_to_csv = BashOperator(
            task_id='law_element_to_csv',
            bash_command=f'sqlite3 {FILE_SQLITE_DB} -header -csv "SELECT * FROM law_element;" > {FILE_LAWS_CSV}'
        )

        case_law_to_csv = BashOperator(
            task_id='case_law_to_csv',
            bash_command=f'sqlite3 {FILE_SQLITE_DB} -header -csv "SELECT * FROM case_law;" > {FILE_CASELAW_CSV}'
        )

        law_alias_to_csv = BashOperator(
            task_id='law_alias_to_csv',
            bash_command=f'sqlite3 {FILE_SQLITE_DB} -header -csv "SELECT * FROM law_alias;" > {FILE_LAWALIAS_CSV}'
        )
