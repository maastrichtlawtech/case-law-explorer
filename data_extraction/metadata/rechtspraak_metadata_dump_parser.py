...
def stringify_children(node):
	...
	parts = ([node.text] + list(chain(*([c.text, tostring(c, encoding=str), c.tail] for c in node.getchildren()))) + [node.tail])
...
def write_data_to_file(data,filename):
	csv_columns = ['id','date','case_number','description','language','venue','abstract','procedure_type','lodge_date','country','subject','authority','legal_references','related_cases','alternative_sources']
...
# Index of files to parse (we sample 10 files from each directory if possible)
...
for directory in dirs:
	...
	done = (xml_count >= 200) or (file_index == len(files_in_dir)-1) or (len(files_in_dir) == 0)
	while done == False:
		current_file = files_in_dir[file_index]
		if current_file[-4:].lower() == ".xml":
			list_of_files_to_parse.append(directory+"/"+current_file)
			xml_count+=1
		file_index+=1
		done = (xml_count >= 200) or (file_index == len(files_in_dir)-1) or (len(files_in_dir) == 0)
